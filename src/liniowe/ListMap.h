...
private:
	...
	~CCount() {
//		std::cout << count << " deleting: " << this << std::endl;
		assert(count > 0);
		//if(count>0)
		count--;
		//else
...
struct ListNode: CCount {
	...
	ListNode(const std::pair<int, std::string>& d) :
			next(NULL), prev(NULL), data(d) {
	}
	ListNode(const std::pair<int, std::string>& d, ListNode* n, ListNode* p) :
...
public:
	...
	/// Zwraca true je�li mapy zwieraj� takie same pary klucz-warto��.
	inline bool operator==(const ListMap& a) const {
		return info_eq(a);
	}
};
